...
from django.shortcuts import render
from . models import details , medicines

import openai 
openai.api_key = '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'
...
def cardio_severity(request):
    data = details.objects.filter(Disease='Heart attack',Severity='Malignant').order_by('-Per').values()
    
    return render(request,"cardio_severity.html",{'data': data})
...
def cardio_disease(request):
    ...
    result("Heart attack",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='HeartAttack',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='HeartAttack',Severity='Benign').medic


    return JsonResponse({ "restext": text, "treatment": med },status = 200)

@login_required(login_url='login')
def jaundice_severity(request):
    data = details.objects.filter(Disease='Jaundice',Severity='Malignant').order_by('-Per').values()#fetch data about severe persons from database for Jaundice disease and sending data again to html file
    return render(request,"jaundice_severity.html",{'data': data})
...
def jaundice_disease(request):
    ...
    result("Jaundice",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='Jaundice',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='Jaundice',Severity='Benign').medic
...
def allergy_severity(request):
    data = details.objects.filter(Disease='Allergy',Severity='Malignant').order_by('-Per').values()
    return render(request,"allergy_severity.html",{'data': data})
...
def allergy_disease(request):
    ...
    result("Allergy",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='Allergy',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='Allergy',Severity='Benign').medic
...
def cold_severity(request):
    data = details.objects.filter(Disease='Common Cold',Severity='Malignant').order_by('-Per').values()
    return render(request,"cold_severity.html",{'data': data})
...
def cold_disease(request):
    ...
    result("Common Cold",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='CommonCold',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='CommonCold',Severity='Benign').medic
...
def diabetes_severity(request):
    data = details.objects.filter(Disease='Diabetes',Severity='Malignant').order_by('-Per').values()
    return render(request,"diabetes_severity.html",{'data': data})
...
def diabetes_disease(request):
    ...
    result("Diabetes",t1,predicted[1] )

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='Diabetes',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='Diabetes',Severity='Benign').medic
...
def thyroid_severity(request):
    data = details.objects.filter(Disease='Hypothyroidism',Severity='Malignant').order_by('-Per').values()
    return render(request,"thyroid_severity.html",{'data': data})
...
def thyroid_disease(request):
    ...
    result("Hypothyroidism",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='Thyroid',Severity='Malignant').medic
    else:
         med = medicines.objects.filter(Disease='Thyroid',Severity='Benign').medic
...
def hepatitis_severity(request):
    data = details.objects.filter(Disease='Hepatitis',Severity='Malignant').order_by('-Per').values()
    return render(request,"hepatits_severity.html",{'data': data})
...
def hepatitis_disease(request):
    ...
    result("Hepatitis",t1,predicted[1])

    if(text == "Malignant - Immediate treatment is required"):
        med = medicines.objects.filter(Disease='Hepatitis',Severity='Malignant').medic
    else:
        med = medicines.objects.filter(Disease='Hepatitis',Severity='Benign').medic
